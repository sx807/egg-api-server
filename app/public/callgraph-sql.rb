...
module Creat
	...
	def Creat.nodes() # 判断输入路径，创建节点，扫描上下级节点
		path=[]
	...
	def Creat.graph2json()
		...
		for j in 1..($number_path)
			for i in $number_in[j-1]..($number_in[j]-1)
				nodes.push({:id => "#{$sline[i]}", :name => "#{$sline[i]}", :groupId => "#{$node_color[j-1]}"})
				# wfile.print %Q{"#{$sline[i]}"}+"[tooltip="+%Q{"#{$sline[i]} #{$sline_time[i]},#{$sline_power[i]}"}+"]"+"[label="+%Q{"#{$sline[i]}"}+","
				# watch=$url_call+"v="+$version+"&f="+$f_vir+"&a="+$a_ver+"&path0="+"#{$sline[i].strip}"+"&path1="
				# wfile.puts "color=#{$node_color[j-1]},style=filled,URL="+%Q{"#{watch}"}+"];" 
			end
		...
		for i in 0..100000
			if $num[i]!=0 or $num_d[i]!=0
				...
				edges.push({:source => $sline[s_j], :target => $sline[b_number], :sourceWeight => $num[i], :targetWeight => 1, :groupId => color})
			end
		end
		wfile.puts JSON.generate({:nodes => nodes, :edges => edges})
		# wfile.puts "}"
	end
end
